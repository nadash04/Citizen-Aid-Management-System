...
def open_edit_citizen_screen():
    ...
    # Store the found citizen's internal ID
    found_citizen_internal_id = None

    def search_citizen():
        nonlocal found_citizen_internal_id
        ...
        try:
            ...
            result_label.config(text=f"Error reading citizen data: {e}", fg="red")
            edit_frame.pack_forget()
            found_citizen_internal_id = None

    def save_changes():
        if found_citizen_internal_id is None:
            messagebox.showerror("Error", "No citizen selected. Please search first.", parent=win)
            return
        ...
        if aid_success and msg_success:
            messagebox.showinfo("Success", "Aid record and message saved successfully!", parent=win)
            message_text.delete("1.0", tk.END)
            next_date_entry.delete(0, tk.END)
        else:
            messagebox.showerror("Error", "Failed to save aid record or message.", parent=win)

    tk.Button(win, text="Search", command=search_citizen, bg="#2196F3", fg="white",
              font=("Helvetica", 10, "bold")).pack(pady=10)
    tk.Button(edit_frame, text="Save Aid Record / Message", command=save_changes,
